...
Protein-centric CAFA Fmax as a function of the maximum sequence identity between each test protein and the training set, computed by BLAST. Bins are defined on the proteins alone, never on any model's predictions, so no method is favoured by the binning. This is the central generalisation test: the "no hit" bin (612 of 754 test proteins) contains proteins with no detectable training homolog, where homology-transfer baselines have nothing to transfer from.{flagged} Bin sizes: {counts_for('homology_bin_counts')}.
