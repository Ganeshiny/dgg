...
def main():
    ...
    for ont, vals in rows.items():
        with (a.tuning_root/"datasets"/f"{ont}_train.pkl").open("rb") as f: _,terms,ytrain=unpack_dataset(pickle.load(f))
        ...
        for t in np.linspace(0,1,101):
            pred=score>=t;ru=np.sum((y & ~pred)*ic,axis=1).mean();mi=np.sum((~y & pred)*ic,axis=1).mean();s=float(np.hypot(ru,mi))
